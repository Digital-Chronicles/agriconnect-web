...
function normalizePin(row: ProduceRow, userLat: number | null, userLng: number | null): ProductPin | null {
...
export default function DiscoverPage() {
  ...
  const pins = useMemo(() => {
    ...
    const mapped = rows.map((r) => normalizePin(r, ulat, ulng)).filter(Boolean) as ProductPin[];
  ...
  const handlePinClick = (_pinId: string, lat: number, lng: number) => {
    if (!mapRef.current) return;
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={() => setRadiusKm(20)} className="text-sm text-emerald-600 hover:text-emerald-700">
                      Reset
                    </button>
                  </div>
                  <input
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
              ...
              <div className="h-[400px] md:h-[500px] w-full relative">
                ...
                <div className="absolute bottom-4 right-4 flex flex-col gap-2 z-[1000]">
                  <button onClick={zoomIn} className="w-10 h-10 bg-white rounded-lg shadow-md flex items-center justify-center">
                    <ZoomIn className="w-5 h-5 text-gray-700" />
                  </button>
                  <button onClick={zoomOut} className="w-10 h-10 bg-white rounded-lg shadow-md flex items-center justify-center">
                    <ZoomOut className="w-5 h-5 text-gray-700" />
                  </button>
                  {userLoc && (
                    <button onClick={goToMyLocation} className="w-10 h-10 bg-white rounded-lg shadow-md flex items-center justify-center">
                      <Target className="w-5 h-5 text-gray-700" />
                    </button>
                  )}
                  <button onClick={resetView} className="w-10 h-10 bg-white rounded-lg shadow-md flex items-center justify-center">
            ...
            <div className="h-[400px] md:h-[500px] overflow-y-auto">
              {loading ? (
                ...
                <div className="divide-y divide-gray-100">
                  {pins.map((pin) => (
                    <div key={pin.id} className="p-4 hover:bg-gray-50 cursor-pointer" onClick={() => handlePinClick(pin.id, pin.lat, pin.lng)}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-2 flex items-center justify-between">
                            <span className="font-bold text-emerald-600 flex items-center gap-1">
                              ...
                              {formatPrice(pin.price_per_unit)}
                            </span>
                          ...
                          <div className="mt-3 flex gap-2">
                            <Link
                              ...
                              className="flex-1 text-center bg-emerald-500 text-white py-2 rounded-lg text-sm hover:bg-emerald-600"
